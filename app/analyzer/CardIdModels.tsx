...
export function CardIdModels ({img,item}:{img:string,item:PropsMappingIAs}){
  const [isLoading,setIsLoading] = useState<boolean>(false)
  const [data,setData] = useState<any>(null)

  const runAnalizer= async ()=>{
    console.log("start analize")
    setIsLoading(true)
    await analizerIA(img,item.model)
    .then(res=>res == undefined ? 
      setData("erro ao analizar") 
      : setData(res)
    )
    .catch(err=>setData(err))
    .finally(()=>setIsLoading(false))
    console.log("end analize")
  }
  ...
  const title = ()=>{
    ...
    return (
    <View
      style={{flexDirection:"row", justifyContent:"space-between",alignItems:"center",gap:30}}
    >
  ...
  return(
    <View>
      <Collapsible title={title()}>
        ...
        {isLoading == false && <Button onPress={()=>runAnalizer()} disabled={isLoading}>{isLoading? <ActivityIndicator/>:"Analizar"}</Button>}
      </Collapsible>
    </View>
  )
}